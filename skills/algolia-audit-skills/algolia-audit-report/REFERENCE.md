# algolia-audit-report — Full Methodology Reference
*Detailed instructions for Phase 3-5 execution. Referenced by SKILL.md.*
*See SKILL.md for trigger, input/output contract, and execution summary.*
...
```bash
export ALGOLIA_AUDIT_DIR=$ALGOLIA_AUDIT_DIR
...
> Surface this section ONLY when `03-traffic-data.md` contains referral, outgoing, or paid search data.
> These signals are often the most persuasive in a discovery call — read them from the file, do not reconstruct.

### Search Abandonment
{If `search_abandonment.google_outbound_pct` is present}
**{X}% of {Company}'s outbound traffic goes to Google** ({mom_change} MoM) — users typing queries into {Company}'s site, failing to find what they need, and going back to Google. At {monthly_visits} monthly visitors, this represents millions of sessions where on-site search broke the journey.

### AI Discovery Shift
{If `ai_referral.chatgpt_pct` is present}
**ChatGPT is sending {chatgpt_pct} of {Company}'s referral traffic** ({chatgpt_mom} MoM). AI Chatbots and Tools represent {total_ai_pct} of all referring industry traffic. {Company}'s buyers are discovering and researching products via AI before they ever reach the site — Algolia's product catalog indexing and API-first architecture directly improves how this traffic converts.

### Paid Search Intelligence
{If `paid_search` is present}
Paid search is {share_of_total_pct} of total traffic. Top non-branded paid keywords: {list top 3}. {If competitor_bidding: "{Company} is actively bidding on '{competitor_bidding}' — a direct competitor conquest strategy. Algolia improves the on-site conversion rate of these intercept arrivals, making the paid spend more efficient."}

### Referral Sources (Top 5)
{Table of top_referrers: domain | share | MoM}

### Competitor Traffic Scale
{Table from competitor_traffic: domain | 3-month visits}
...
| `traffic` | `03-traffic-data.md` | COPY-PASTE exact percentages. If SimilarWeb blocked, set null and explain in `source_url`. Include all sub-fields below. |
| `traffic.search_abandonment` | `03-traffic-data.md` — Outgoing Traffic section | google.com outbound %, MoM change, 1-sentence narrative. Omit if not in file. |
| `traffic.ai_referral` | `03-traffic-data.md` — Referrals section | chatgpt.com %, Gemini %, total AI industry %, 1-sentence narrative. Omit if not in file. |
| `traffic.top_referrers` | `03-traffic-data.md` — Top Referring Websites table | Array of {domain, share_pct, mom_change}. Top 5 max. |
| `traffic.paid_search` | `03-traffic-data.md` — Paid Search section | share_of_total_pct, top 5 keywords, competitor_bidding if present, narrative. |
| `traffic.competitor_traffic` | `03-traffic-data.md` — Competitor Traffic Comparison table | Array of {domain, visits_3mo}. |
...
| `strategic_angles[]` | `06-strategic-context.md` + `12-icp-priority-mapping.md` | EXTRACT — do not synthesize. Open `06-strategic-context.md`, section "Algolia-Specific Strategic Angles". For each angle heading, extract all 8 required fields: `label`, `hook`, `pain_points[]`, `discovery_question`, `algolia_proof`, `objection`, `objection_counter`, `source`. See extraction rules below. |

**Strategic angle rules — MANDATORY:**

**EXTRACT, do not synthesize.** All angles MUST come directly from `06-strategic-context.md`. Do NOT invent new angles. Do NOT substitute different angles. The angles in that file were researched and written by a prior skill — your job is faithful extraction into JSON, not re-interpretation.

**Field extraction mapping** (for each angle in `06-strategic-context.md` section "1. Algolia-Specific Strategic Angles"):
- `label` → the angle heading (e.g. "The NikeAI Gap: App Has AI Search, Website Does Not")
- `hook` → copy the **"Pitch angle:"** sentence verbatim — do not paraphrase
- `pain_points[]` → the bullet evidence items listed under the angle body
- `discovery_question` → one question derived from the angle's evidence (may be explicitly stated or requires brief synthesis from the evidence)
- `algolia_proof` → the Algolia case study metric cited in the angle body (e.g. "+35% conversion — Under Armour")
- `objection` → from section "7. Objection Pre-Emption" table — the likely objection row matching this angle's theme
- `objection_counter` → the corresponding evidence response from that same table row
- `source` (REQUIRED) → the [FACT — ...] citation from the markdown body

**NEVER write an angle based on inferred or assumed facts.** If an angle is in `06-strategic-context.md`, extract it. If it is not, do not add it.

Example valid source: `"source": "[FACT — Q3 FY2025 earnings call WebFetch, designerbrands.com/ir, 2026-03-21]"`
Example invalid source: `"source": null` or `"source": ""` — these are BLOCKING violations.

All 8 fields are REQUIRED. An angle missing any field will fail schema validation and block the render.
| `icp_mapping` | `12-icp-priority-mapping.md` | Full object: `priority_to_product[]`, `outreach_sequence[]`, `anchor_lines[]` |
| `competitive_synthesis` | `04-competitors.md` + research | `scenario`, `competitor_tiers[]`, `experience_debt{}`, `positioning_matrix[]`, `positioning_statements{}`, `the_ask`, `roi_by_scenario{}`, `matrix_col_labels{}` |

**Capability matrix rules — MANDATORY:**

The `positioning_matrix` is a sales tool. Its purpose: what the prospect can't do today → what a reachable competitor does → what Algolia enables.

1. **Only include reachable competitors in comparison columns.** If a competitor uses a proprietary/unavailable tool (e.g., Amazon-owned infrastructure for Zappos), exclude them from the matrix comparison columns. Showing what the prospect can never buy misleads the AE. If needed for context, put them in a separate "benchmark" note outside the matrix.

2. **Set `matrix_col_labels` — never leave it null.** Map the 4 canonical column keys (nike_has, asics_has, on_running_has, new_balance_has) to the actual competitor names for this company. Null `matrix_col_labels` renders the Brooks Running defaults (Nike/Asics/etc.) regardless of the actual company.
...
- `id`: F01, F02, ... in severity order (critical first, then moderate, then positive)
...
**Finding coverage requirement — MANDATORY:**
Write one finding per scoring area that scored < 8, regardless of severity.
- HIGH severity areas scoring < 5 → severity = `critical`
- MEDIUM severity areas scoring < 8 → severity = `moderate`
- Any area scoring ≥ 8 → no gap finding needed (may add a `positive` finding)

**You must produce moderate findings.** If 4 areas scored MEDIUM, you must write 4 moderate findings. Do not write only critical findings and stop. A report with zero moderate findings is INCOMPLETE — it misrepresents the audit scope.

**Case study diversity — MANDATORY:**
- Each finding must use a DIFFERENT Algolia case study where possible.
- Do NOT point every finding at the same case study URL (e.g., Shoe Carnival for every finding).
- Check `04-competitors.md` for all identified Algolia customers, AND check `algolia.com/customers/` for same-vertical case studies.
- Match the case study to the finding type: NLP gap → NLP case study, personalization gap → personalization case study, federated search gap → federated search case study.
- If only one same-vertical case study exists, use it for the most relevant finding and use general-vertical case studies for others.
...
| `search-abandonment` | `03-traffic-data.md` — Outgoing Traffic | If google.com outbound % is present: use as type `industry-risk`. Signal = "{X}% of outbound traffic exits to google.com ({mom} MoM) — users abandon on-site search". Relevance: "Quantified proof search is failing; Algolia reduces this exit rate." |
| `ai-discovery` | `03-traffic-data.md` — Referrals | If chatgpt.com referral % is present: use as type `industry-opp`. Signal = "ChatGPT sends {X}% of referral traffic ({mom} MoM); AI Chatbots = {total}% of referring industries". Relevance: "AI discovery shift requires catalog-quality search to convert AI-referred intent." |
| `paid-conquest` | `03-traffic-data.md` — Paid Search | If competitor_bidding is present: use as type `competitor`. Signal = "Bidding on '{competitor}' — active paid conquest of competitor's brand traffic". Relevance: "Algolia improves on-site conversion of these high-intent intercept arrivals." |
...
assert len(d['findings']) >= 5, f'only {len(d[\"findings\"])} findings — need one per gap area'
assert all(f.get('screenshot_file') for f in d['findings']), 'finding missing screenshot_file'
sev = d.get('score', {}).get('breakdown_severity', {})
medium_areas = [k for k,v in sev.items() if v == 'MEDIUM']
moderate_findings = [f for f in d['findings'] if f.get('severity') == 'moderate']
if medium_areas and not moderate_findings:
    raise AssertionError(f'MEDIUM areas {medium_areas} have no moderate findings')
case_urls = [f.get('algolia_case_study_url','') for f in d['findings'] if f.get('severity') not in ('positive','')]
if len(case_urls) > 1 and len(set(case_urls)) == 1:
    raise AssertionError(f'All findings use same case study — diversify by finding type')
print(f'✓ {len(d[\"findings\"])} findings ({len(moderate_findings)} moderate), {len(d.get(\"intelligence_signals\", []))} signals')
"
```
...
## AE Playbook Structure (BLUF + MEDDPICC)

The AE Playbook (`{slug}-playbook.md`) merges the AE Pre-Call Brief with sales plays. Structure:

1. **BLUF** (Bottom Line Up Front) — 3 bullets: gap, opportunity, ask
2. **MEDDPICC Mapping** — M: metrics (ROI), E: economic buyer, D: decision criteria, D: decision process, P: paper process, I: identify pain, C: champion, C: competition
3. **Top 3 Discovery Questions** — derived from "In Their Own Words" + audit findings
4. **Objection Counters** — pre-built from `hiring.objection_counters[]` in audit-data.json
5. **ROI Framing by Scenario** — conservative/moderate/optimistic with payback period
6. **Competitive Positioning** — golden angle + differentiation from battle card
7. **Pilot Blueprint** — 3-phase (Foundation → Optimize → Scale) with KPIs per phase
8. **Next Actions** — 3 specific CRM-ready action items
