...
CANONICAL_TOP_LEVEL = {
    ...
    # Previously undocumented but in active use (8)
    'competitive_synthesis','golden_angle','strategic_angles','hiring','icp_mapping',
    'abx_sequence','case_studies','demos',
    # Newly added (3)
    'partner_intel','tab_subtitles','recommended_first_play',
    # Tavily signals build (1)
    'industry_context',
}
...
APPROVED_CSS_CLASSES = {
    # From algolia-brand.css
    'dp-tile','card-3d','card-3d--no-lift',
    'glow-card','glow-active',
    'bounce-card','bounce-card--red','bounce-card--amber','bounce-card--green',
    'bounce-card__content','bounce-card__gradient','bounce-card__amount','bounce-card__amount-sub',
    'feature-grid','feature-card',
    'feature-card--high','feature-card--medium','feature-card--positive',
    'feature-card--urgent','feature-card--warning',
    'fc-icon','fc-title-row','fc-accent-bar','fc-title','fc-desc','fc-source',
    'ab-container','ab-highlight','ab-item',
    'proof-pill','proof-pill--green','proof-pill--blue','proof-pill--white',
    'proof-pill__badge','proof-pill__label',
    'unverified-warning','unverified-wrapper',
    # From index-template.html inline styles
    'inline-src','src-tag',
    'severity--high','severity--medium','severity--low',
    'annotated-screenshot','annotation-circle','annotation-callout',
    'finding-card-gradient__title','finding-card-gradient__desc',
    # New navigation system (added 2026-03-23)
    'tab-subtitle',
    'section-strip','section-strip__pill','section-strip__pill--active',
    'highlights-grid','highlight-card','highlight-card__icon','highlight-card__stat','highlight-card__label',
    'section-subtitle',
}
...
def check_inline_styles(workspace):
    ...
    # ALL deliverables/*.html files are compiled T.* outputs — T.* tokens expand to inline styles
    # by design. check-style-tokens.py covers template source compliance.
    # Inline style enforcement applies to source files only, not rendered deliverables.
    files = []  # skip compiled HTML outputs
...
def check_path_convention(workspace):
    ...
    # Also check skills directory scripts (excluding this script itself to avoid self-reference)
    skills_scripts = Path.home() / '.claude/skills/algolia-search-audit/scripts'
    if skills_scripts.exists():
        all_files += [f for f in skills_scripts.glob('*.py') if f.name != 'algolia-standards-check.py']
