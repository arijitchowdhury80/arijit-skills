# algolia-audit-factcheck — Full Methodology Reference
*Detailed 20-dimension verification procedures. Referenced by SKILL.md.*
*See SKILL.md for trigger, tier options, and output contracts.*
...
**If ALGOLIA_AUDIT_DIR is not set:** Use current working directory as the base. Run: `export ALGOLIA_AUDIT_DIR="$(pwd)"` or set it to your preferred audit storage location.
...
1. **Locate audit files** — Scan the provided directory for:
   ...
   - **Workspace** (11+ files): `$ALGOLIA_AUDIT_DIR/{CompanyName}/research/` containing `01-company-context.md` through `11-investor-intelligence.md` plus `_workspace-manifest.md`
...
4. **Create factcheck workspace**:
   ```
   $ALGOLIA_AUDIT_DIR/{CompanyName}/factcheck/
...
After all agents complete, the team lead (main conversation) collects all dimension results and generates the 4 output files.
...
**Critical design principle**: This file provides the VERIFIED DATA, not the corrections. The audit skill must figure out WHERE in its own files the wrong data lives and HOW to fix it.

---

### Output 1: `{company}-factcheck-report.md`

**Write to:** `$ALGOLIA_AUDIT_DIR/{CompanyName}/deliverables/{company}-factcheck-report.md`
...
**Write to:** `$ALGOLIA_AUDIT_DIR/{CompanyName}/deliverables/{company}-correction-manifest.md`
...
**Write to:** `$ALGOLIA_AUDIT_DIR/{CompanyName}/deliverables/{company}-skill-feedback.md`
...
TEAM LEAD (Coordinator — this conversation)
  ...
  │
  │ Phase 3 (team lead)
  └─→ Score aggregation + 4 output files
```
...
**Phase 3 — Team Lead**:
Read all 5 dimension result files. Aggregate scores. Generate 4 output files.
...
**Location**: `$ALGOLIA_AUDIT_DIR/{CompanyName}/research/FACTCHECK_GATE.md`
...
8. **4 output files** — verification data + human report + machine-readable corrections + methodology feedback creates a full improvement loop
...
/algolia-search-audit  ──→  6 deliverables + workspace
         │
         ▼
/algolia-audit-factcheck  ──→  4 files:
         │                      ├── verification-data.md (verified ground truth)
