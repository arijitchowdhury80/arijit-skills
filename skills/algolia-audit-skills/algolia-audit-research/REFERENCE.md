# algolia-audit-research — Full Methodology Reference
*Detailed instructions for all 14 research steps. Referenced by SKILL.md.*
*See SKILL.md for trigger, input/output contract, and step summary table.*
...
Write `{CompanyName}/research/CHECKPOINT.md` immediately at the start (before any API calls) and update after EVERY completed step.
...
Update the step line to `[x] Step N: Name — DONE (YYYY-MM-DD HH:MM)` after each step completes. Update `Last Updated` timestamp and add blockers to Notes.

---

## Source Recording Gate (applies to EVERY step — MANDATORY)

Before marking any step as DONE in CHECKPOINT.md:

1. Count the source URLs written to the output file for this step:
   ```bash
   grep -c 'https://' {output-file}.md
   ```
2. If count = 0: the step is NOT done. Go back and add source URLs to every data point.
3. If count > 0: record the count in CHECKPOINT.md:
   `[x] Step N: {Name} — DONE | Sources: {count} URLs logged`

**Minimum source counts per step:**
...
| 10 Trigger Events | 2 | News articles with publication dates |
| 11 Case Studies | 2 | algolia.com/customers/ URLs (WebFetch verified) |
| 12 Investor Intel | 3 | Transcript URL + 10-K URL + Yahoo Finance |
| 13 Deep Hiring | 2 | Careers page + job posting URLs |
| 14 ICP Mapping | 0 | (synthesis of existing data, no new sources) |
...
**Citation format**: Inline markdown hyperlinks `[Source](URL)` on every data point. A scratchpad file without sources is INCOMPLETE.
...
**Tag rules**: An [ESTIMATE] must NEVER be presented as a [FACT] in deliverables. An [OBSERVED] value from the browser must not be inflated. Deliverables must preserve these distinctions.

### Data Tier Labels — Apply at Collection Time (MANDATORY)

Every data point written to any scratchpad file MUST be tagged with one of three tiers at the moment it is written. Do NOT wait for factcheck to assign tiers.

| Label | When to use | Example |
|-------|-------------|---------|
| `[FACT — {source}, {date}]` | From MCP API call, SEC filing, official company IR, official press release | `Revenue: $275.2B [FACT — Yahoo Finance MCP, 2026-03-20]` |
| `[ESTIMATE — {source}, {date}]` | From WebSearch, WebFetch of third-party article, analyst estimate, ecdb/Crunchbase | `Revenue: ~$550M [ESTIMATE — Crunchbase, 2026-03-20]` |
| `[OBSERVED — Phase 2, {date}]` | Directly observed during browser audit | `Empty state shows no suggestions [OBSERVED — Phase 2, 2026-03-20]` |

**Rule:** An [ESTIMATE] must NEVER be presented without its label in any deliverable. The factcheck verifies existing labels — it does not assign them.

**Why at collection time, not factcheck time:** By factcheck time, context has degraded and the source is harder to trace. If you don't know the source when you write the data point, you will not know it 14 steps later.
...
- Earnings call transcripts (SEC filings are fetched directly via WebFetch on sec.gov)
...
**Gold standard example — SimilarWeb unavailability**: If SimilarWeb returns no data:
1. Document the failure explicitly: `SimilarWeb traffic: NO DATA (API returned error — site may not be indexed)`
...
## Wave Architecture (Parallel Execution — claudesp only)
...
**Prerequisites for parallel mode**: Install `claude-sneakpeek` to unlock Agent Teams tools: `npx @realmikekelly/claude-sneakpeek quick --name claudesp`. Run via `claudesp` (not `claude` or VS Code extension). Teammates inherit the lead's permissions at spawn time.
...
Create `_workspace-manifest.md` with all 14 steps listed as `[ ] pending`:
...
> **Pattern per step**: Run MCP/API call → Write structured results to scratchpad file → Update CHECKPOINT.md → Continue to next step.
>
...
>
> **Tag rules**: An [ESTIMATE] must never be presented as a [FACT] in deliverables. Deliverables must preserve these distinctions.
...
For the top 3-5 competitors, estimate their search experience maturity on the same 0-10 scale used in Phase 3 scoring. Base estimates on:
...
Legend: ✅ = Present/Strong | ⚠️ = Partial/Basic | ❌ = Absent/Weak
...
4. Verify that the event described matches the timeline claimed in your deliverables
...
**TWO-SOURCE REQUIREMENT (mandatory — both sources required for public companies):**

Source A — Yahoo Finance MCP: ALL financial numbers
  Use: get_stock_info, get_financial_statement (income+balance+cashflow+quarterly), get_recommendations, get_historical_stock_prices, get_yahoo_finance_news
  What it gives: Revenue, margins, EPS, analyst ratings, stock data, recent news feed
  Label all output: [FACT — Yahoo Finance MCP, {date}]

Source B — Transcript WebFetch: ALL verbatim executive quotes
  Use: WebFetch on Motley Fool (fool.com/earnings/call-transcripts/...) or Seeking Alpha or company IR page
  What it gives: Exact words executives said on earnings calls, investor days, conferences
  Label all output: [FACT — {source name} transcript WebFetch, {date}]

**NEVER substitute one for the other:**
- Yahoo Finance news summaries ≠ verbatim transcript quotes (summaries are paraphrases)
- Transcript WebFetch ≠ financial data (transcripts discuss strategy, not verified financial figures)
- Both are required. If transcripts are unavailable, note it and use [non-verbatim] notation — never use quote marks without WebFetch confirmation.

**For public companies** (verified filings via **WebFetch on sec.gov** + **Yahoo Finance MCP**):

a. **Retrieve Latest 10-K AND 10-Q** — WebFetch the SEC EDGAR filing directly:
   - Find 10-K: WebFetch `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={ticker}&type=10-K&dateb=&owner=include&count=5`
   - Find 10-Q: WebFetch `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={ticker}&type=10-Q&dateb=&owner=include&count=5`
   - From the results page, find the most recent filing link and WebFetch the document index
   - Then WebFetch the actual 10-K/10-Q document to read MD&A, Risk Factors
   - Label all data extracted this way as [FACT — SEC EDGAR WebFetch, {date}] with the exact URL

b. **Extract Strategic Narrative (MD&A)** — WebFetch the 10-K/10-Q document retrieved in step (a):
   - Read Item 7 (MD&A) from 10-K AND Item 2 (MD&A) from 10-Q
...
c. **Extract Risk Factors** — WebFetch the 10-K document retrieved in step (a):
