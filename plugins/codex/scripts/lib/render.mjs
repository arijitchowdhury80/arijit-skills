...
function normalizeReviewFinding(finding, index) {
  ...
  return {
    ...
    tier: source.tier === "confirmed" || source.tier === "suspected" ? source.tier : null,
...
function normalizeCoverageEntry(entry) {
  ...
  return {
    unit: typeof source.unit === "string" && source.unit.trim() ? source.unit.trim() : "unknown",
    status:
      source.status === "checked-clean" || source.status === "at-risk" || source.status === "cannot-assess"
        ? source.status
        : "cannot-assess",
...
function normalizeReviewResultData(data) {
  return {
    ...
    coverage: Array.isArray(data.coverage) ? data.coverage.map((entry) => normalizeCoverageEntry(entry)) : [],
...
export function renderReviewResult(parsedResult, meta) {
  ...
  if (findings.length === 0) {
    ...
    for (const finding of findings) {
      ...
      const tierTag = finding.tier ? `[${finding.tier}]` : "";
      const confTag = typeof finding.confidence === "number" ? ` (confidence: ${finding.confidence})` : "";
      lines.push(`- [${finding.severity}]${tierTag} ${finding.title} (${finding.file}${lineSuffix})${confTag}`);
  ...
  if (data.coverage.length > 0) {
    lines.push("", "Coverage (every reviewed unit):");
    for (const entry of data.coverage) {
      lines.push(`- [${entry.status}] ${entry.unit}${entry.reason ? ` — ${entry.reason}` : ""}`);
    }
  }
