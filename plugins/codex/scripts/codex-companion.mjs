...
const MAX_REVIEW_SCHEMA = path.join(ROOT_DIR, "schemas", "review-output-max.schema.json");
...
function buildAdversarialReviewPrompt(context, focusText, options = {}) {
  const templateName = options.templateName ?? "adversarial-review";
  const reviewKind = options.reviewKind ?? "Adversarial Review";
...
async function executeReviewRun(request) {
  ...
  const isMax = reviewName === "Adversarial Review Max";
  const prompt = buildAdversarialReviewPrompt(context, focusText, {
    templateName: isMax ? "adversarial-review-max" : "adversarial-review",
  ...
  const result = await runAppServerTurn(context.repoRoot, {
    ...
    model: request.model,
    sandbox: "read-only",
    outputSchema: readOutputSchema(isMax ? MAX_REVIEW_SCHEMA : REVIEW_SCHEMA),
    onProgress: request.onProgress
  });
...
// Native `/codex:review` removed in the Arijit fork. The native-review helpers
// (validateNativeReviewRequest, buildNativeReviewTarget, the "Review" branch in
// executeReviewRun) are left dormant/unreachable — no user-facing command reaches them.
...
async function main() {
  ...
  switch (subcommand) {
    ...
    case "adversarial-review":
      ...
      break;
    case "adversarial-review-max":
      await handleReviewCommand(argv, {
        reviewName: "Adversarial Review Max"
      });
      break;
    case "task":
